...
MediaItemWidget::MediaItemWidget(QWidget* parent, MediaItem* item,
	...
	:	ItemWidget(parent, highlightable),
		...
		myName(new TextItemWidget(this, "",
				AWEMC::settings()->getFontByName("normal"))),
...
{
	DEBUG_OUT("Trying to fit \"" << myName->getText().toStdString()
				<< "\" " << myDisplayMode << " into size (" << size.width()
				<< ", " << size.height() << ")")
	myFitInSize = size;
	bool ans;
	int minw, minh;
...
void MediaItemWidget::enactDisplayMode()
{
	switch (myDisplayMode)
	{
		...
		case NameAndIconTopToBottom:
			...
			break;
		default:
			DEBUG_ERR("ERROR: No match for MediaItem display mode")
			break;
	}
	update();
}