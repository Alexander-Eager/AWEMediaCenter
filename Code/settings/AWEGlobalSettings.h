...
class AWEMetadataScraper;
class AWEMediaItem;
class AWEFolder;
class AWEMediaService;
...
class AWEGlobalSettings
{
	public:
		 ...
		 **/
		AWEGlobalSettings(const std::string& settingsFile);
		 ...
		 *
		 * \returns The desired scraper as an `AWEMetadataScraper` object
		 *			or `NULL` if it does not exist.
		 **/
		AWEMetadataScraper* getScraperByName(const std::string& name);
		 ...
		 *			or `NULL` if it does not exist.
		 **/
		AWEMediaPlayer* getPlayerByName(const std::string& name);
		 ...
		 **/
		AWEMediaService* getMediaServiceByName(const std::string& name);
		 ...
		 **/
		AWEMediaItem* getMediaItemByJSONFile(const QDir& file);
		 ...
		 **/
		void addFolder(const std::string& path, AWEFolder* folder);
	...
	private:
		...
		std::map<std::string, AWEMediaPlayer*> myMediaPlayers;
		...
		std::map<std::string, AWEMetadataScraper*> myMetadataScrapers;
		...
		std::map<std::string, AWEMediaService*> myMediaServices;

		/** \brief Maps the absolute, clean path of an item onto the item. **/
		std::map<std::string, AWEMediaItem*> myMediaItems;
		/** \brief The root folder. **/
		AWEFolder* myRootFolder;
